 ...
 * 
 * 1. If: If statements contain code that only runs if the statement's condition returns true. If statements are structured like: if (condition) {code to run};
 * 
 * 2. Else-if: Else-if statements contain code that only runs if the statement's condition returns true and if the previous if or else-if statement that the
 * current else-if statement is attached to didn't run its code due to its condition returning false. If statements are structured like: {code block of previous statement} else if (condition) {code to run};
 * 
 * 3. Else: Else statements run their code when every other if and else-if statement in the chain failed to run their code due to failing condition checks that all returned false.
 * Else statements have no condition to check, thus they look like: {code block of previous statement} else {code to run}.
 * 
